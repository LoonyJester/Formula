...
public class CpCalculator
{
	...
	public final double CalculatedFCR(double inletRate, double exchangeVolume, double exchangeVolumeChange, double exchangeHct, double exchangeFcr, double exchangeEndHct, double exchangeTargetFcr, double exchangeRfHct, int flag)
	{
		...
		double f = exchangeRfHct / exchangeHct * ((p - 1) / (p - (exchangeEndHct / exchangeHct)));
		double qrf;
		...
		if (Math.abs(exchangeVolumeChange) > 0.0000001)
		{
			...
			exchangeTime = exchangeVolume / flowRateDifference * ((1 / Math.pow(therapeuticFcr, (1 / (m - 1)))) - 1);
		}
		else
		{
	...
	public final double CalculatedFCRVR(double exchangeHct, double exchangeRfHct, double exchangeEndHct, double exchangeVolumeChange, double exchangeVolume, double exchangeReplacementVolume, double inletRate, int flag)
	{
		if (flag != 1)
		{
			return 0;
		...
		if (g != e)
		{
			...
			do
			{
				...
				double dgF = ((c * Math.log(x) * Math.pow(d - F * g, 2) * Math.pow(x, c * F / a)) + (a * d * (e - g))) / (a * Math.pow((d - (F * g)), 2));
				F0 = F - (gF / dgF);
				n = n + 1;
	...
	public final double[] RBCXTT(double inletRate, double qrf, double qp, double currentRfHct, double modeTime, double initialModeVolume, double initialModeHct, double initialModeFcr)
	{
		...
		if (Math.abs(flowRateDifference) > 0.0000001)
		{
			...
			currentPatientHct = (currentRfHct / f) - (((currentRfHct / f) - initialModeHct) * Math.pow((initialModeVolume / currentPatientVolume), m));
		}
		else
		{
			currentPatientHct = (currentRfHct / f) - (((currentRfHct / f) - initialModeHct) * Math.exp(-inletRate * modeTime / initialModeVolume));
		}

		return new double[] {currentPatientVolume, currentPatientHct, currentFcr};
	...
	public enum ProcedureType
	{
		...
		public static ProcedureType forValue(int value)
		{
			return values()[value];
		}
	}
	...
	public enum WeightType
	{
		...
		public static WeightType forValue(int value)
		{
			return values()[value];
		}
	}
	...
	public enum HeightType
	{
		...
		public static HeightType forValue(int value)
		{
			return values()[value];
		}
	}
	...
	public enum BloodVolumeType
	{
		...
		public static BloodVolumeType forValue(int value)
		{
			return values()[value];
		}
	}
	...
	public enum ReplacementVolumeType
	{
		...
		public static ReplacementVolumeType forValue(int value)
		{
			return values()[value];
		}
	}
	...
	public final String EP(ProcedureType Procedure_Type, GenderType Gender, double Weight, WeightType Weight_Units, double Height, HeightType Height_Units, double Total_Blood_Volume, BloodVolumeType Total_Blood_Volume_Override, double FCR, double Initial_Hct, double Target_End_Hct, double Avg_RF_Hct, double Replacement_Volume, ReplacementVolumeType Replacement_Volume_Override, double Target_Depletion_Hct, double Blood_Warmer_Volume, int Flag)
	{
		...
		if (Predicted_Error.equals("None"))
		{
			if (Procedure_Type == ProcedureType.Depletion)
			{
		...
		if (Flag == 0)
		{
			double r = (Math.pow((Estimation_FCR * (Entered_Patient_Volume / Estimation_Volume) * (Entered_Patient_Hct / Estimation_Hct)), (FCR_Factor))) * 100;
			return String.valueOf(r);
		}
		if (Flag == 1)
		{
			return String.valueOf(Replacement_Volume);
		}

		if (Flag == 2)
		{
			 String.valueOf(Est_Other_RF);
		}
		if (Flag == 3)
		{
